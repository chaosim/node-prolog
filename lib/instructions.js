...
function evaluateInstructions(instructions, HEAP, X) {
	...
	for (var i = 0 ; i < instructionsCount ; i++) {
		instructions[i].eval(HEAP, X, ENV);
	};
}
	...
	this.eval = function(HEAP, X, ENV) {
		...
		switch(addr.label) {
			case "ref":
				...
				ENV.bind(addr.reference, addr, new StoreRef(HEAP, HEAP.H()), HEAP.get(HEAP.H()));
				HEAP.increment();
				HEAP.increment();
				ENV.MODE("WRITE");
...
function unify_value(reference) {
	...
	this.eval = function() {
		console.log("[EVAL]\t\t" + this.inspect());
		throw "Unimplemented Path: EVAL " + this.inspect();
	};
}
