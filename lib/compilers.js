	...
	for(var x = 1 ; x <= queryPredicate.arity ; x++) {
		...
		switch(reg.constructor) {
			...
			case Structures.Variable:
				...
				if(hasEncounteredRef(reg.reference)) {
					...
					// Check the ref map so we know what reference to put in the instruction
					var insRef = refMap[reg.reference];
					if(!insRef) {
						throw "Compiler Error: No Ref found in RefMap. [" + argument.inspect() + "]";
					}
					...
					ENV.setQueryVariableBinding(reg.symbol, new StoreRef(ENV.X(), x));
 ...
 **/
function CompileLoadedFacts(ENV, facts, asRule)  
{
	// TODO: Move this to the corect place.
	...
	function compileStructure(reference, insRef) {
		...
		CODE.pushInstruction(new Instructions.get_structure(structure.symbol, structure.arity, insRef));
	...
	for(var i in facts) {
		var fact = facts[i];
		var factStartingCodeIndex = CODE.length();

		// Push the label into the code area
		if(asRule !== true) {
			CODE.labelInstruction(fact.symbol + "/" + fact.arity, factStartingCodeIndex);			
		}

		/**
		 * Procedure Compilation
		 **/
		if(fact.constructor === Structures.Procedure) {
			CODE.pushInstruction(new Instructions.allocate(fact.permanentVariables.length));

			// Compile the HEAD as a fact
			CompileLoadedFacts(ENV, [fact.head], true);
			
			// Compile each rule in the body as a query
			var numSubterms = fact.body.length;
			for(var x = 0 ; x < numSubterms ; x++) {
				CompileLoadedQuery(ENV, fact.body[x]);
			}

			CODE.pushInstruction(new Instructions.deallocate());
			continue ;
		}
		...
		for(var i = 1 ; i <= arguments_length ; i++) {
			...
			switch(item.constructor){
				case Structures.CompleteStructure:
					compileStructure(ref, new StoreRef(ENV.X(), i));
					break ;
				case Structures.Variable:
					// check if we've encountered this reference before
		...
		while(nonArgumentCompilationStack.length > 0) {
			var ref = nonArgumentCompilationStack.pop();
			compileStructure(ref, ref);
		}
...
module.exports.CompileLoadedFacts = CompileLoadedFacts;
