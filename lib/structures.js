...
function StoreRef(store, index) {
	...
	this.deref = function() {
		...
		// TODO: Find a better way to do this check, comparing the constructors means we can only have one of each type of store at a time.
		if(test.constructor == StoreRef) {
			...
			if(test.label == "ref" || test.label == "str") {
				if(test.reference.store == this.store && test.reference.index == this.index) {
					return this;
				} else {
