...
function rebuildTermFromHeap(reference) {
	...
	for(var i = 0; i < cell.arity; i++) {
		...
		if(argCell.constructor === StoreRef) {
			argCell = argCell.get();
		}
...
function BuildAndRun(DEBUG, prepareQueryCallback, prepareProgramCallback) {
	 ...
	 **/
	Compiler.CompileLoadedFacts(ENV, programTerms);
	 ...
	 **/
	var argValues = [];
	for(var a in startingPredicate.args) {
		argValues.push(this.rebuildTermFromHeap(startingPredicate.args[a].get().reference));
	}
	var result = new CompleteStructure(startingPredicate.symbol, startingPredicate.arity, argValues);
