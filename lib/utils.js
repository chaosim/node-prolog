...
function BuildAndRun(DEBUG, prepareQueryCallback, prepareProgramCallback) {
	 ...
	 **/
	var flattenedQuery = prepareQueryCallback(ENV);

	/**
	 * Compile the argument registers into query instructions
	 **/
	var queryInstructions = Compiler.CompileLoadedQuery(ENV, flattenedQuery);

	/**
	 * Load the program and faltten (provided by callback).
	 **/
	var flattenedProgram = prepareProgramCallback(ENV);

	/**
	 * Compile the argument registers into program instructions
	 **/
	var programInstructions = Compiler.CompileLoadedProgram(ENV.X(), flattenedProgram);
